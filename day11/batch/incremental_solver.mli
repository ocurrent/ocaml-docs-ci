...
(** [reuse_solutions ~solutions_cache_dir ~previous_dir ~changed_packages
    ...
    [?expected_cache_key] additionally requires each previous entry to pass
    {!is_cache_key_valid} against it — pass the cache key the previous
    snapshot's entries were written with, so entries left by an interrupted run
    under different inputs are not trusted.

    [?rekey_to] switches from hardlinking to rewriting: each reused entry is
    re-saved with the given cache key so the destination snapshot's own validity
    check accepts it. In this mode an existing destination file is overwritten
    (the caller is expected to pass only targets it knows to be missing or
    stale). Cached {e failures} are carried like solutions — an untouched
    examined set means the failure provably still holds; whether that
    short-circuits the re-solve is the consumer's call.

    [?yield] is invoked once per package considered — pass a cooperative-yield
    hook when running on an event loop so a large sweep doesn't starve other
    work.

    Reuse composes across snapshots: an entry reused into snapshot N was checked
    against the N-1→N diff and rekeyed, so checking the N→N+1 diff at the next
    hop maintains validity inductively. *)

val tool_solutions_dirname : string
(** Per-snapshot subdirectory holding cached doc-tool solves
    ([<pkg>@<compiler>.json] entries — the compiler pin is encoded in the
    filename; {!load} reads the package from the JSON body). *)

val tool_cache_key : repos:(string * string) list -> string
(** [tool_cache_key ~repos] is the cache key stamped on tool-solve entries: a
    digest of the [(path, sha)] repo set alone. The compiler rides in the
    entry's filename, so one key covers every entry in a snapshot's
    {!tool_solutions_dirname}. *)
