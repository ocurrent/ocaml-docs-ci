    ...
    (fun (path, sha) ->
      Lwt.catch
        ...
        (fun exn ->
          ...
          Logs.warn (fun m ->
              m "oid index: skipping %s: %s" path (Printexc.to_string exn));
          Lwt.return_unit))
    repos_with_shas
  >|= fun () -> tbl

let finalise_load (profile : Profile.t) ~cache_dir ?oid_index git_packages
    repos_with_shas =
  ...
  let find_oid =
    Option.map
      (fun idx -> fun pkg -> Hashtbl.find_opt idx (OpamPackage.to_string pkg))
      oid_index
  in
  let hash_cache =
    Day11_opam_build.Hash_cache.create ~find_opam ?find_oid ?patches ()
  in
  {
    profile;
    cache_dir;
...
let name_caches : (string, Day11_opam.Git_packages.name_cache) Hashtbl.t =
  Hashtbl.create 4
...
let load_lwt (profile : Profile.t) ~cache_dir =
  ...
  let prev =
    List.filter_map
      (fun r -> Option.map (fun c -> (r, c)) (Hashtbl.find_opt name_caches r))
      profile.opam_repositories
  in
  Day11_opam.Git_packages.of_repositories_incremental_lwt ~prev repos_with_heads
  >>= fun (git_packages, repos_with_shas, fresh_caches) ->
  List.iter
    (fun (path, cache) -> Hashtbl.replace name_caches path cache)
    fresh_caches;
  build_oid_index_lwt repos_with_shas >|= fun oid_index ->
  finalise_load profile ~cache_dir ~oid_index git_packages repos_with_shas
