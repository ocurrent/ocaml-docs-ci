...
let test_reuse_rekey () =
    ...
    Incremental_solver.Cached_solution
      {
        ...
        result =
          {
            Day11_solution.Solve_result.packages =
              OpamPackage.Set.singleton (pkg name);
            build_deps = solution;
            doc_deps = solution;
            examined =
              OpamPackage.Name.Set.of_list
  ...
  in
  let reused =
    Incremental_solver.reuse_solutions ~expected_cache_key:"prev-key"
      ~rekey_to:"new-key" ~solutions_cache_dir:cur_dir ~previous_dir:prev_dir
      ~changed_packages:changed
      ~packages:[ "astring.0.8.5"; "fmt.0.9.0"; "broken.1.0" ]
      ()
  in
  ...
  | Ok (Cached_solution s) ->
      Alcotest.(check (option string))
        "re-stamped key" (Some "new-key") s.cache_key
  | _ -> Alcotest.fail "expected rekeyed solution in cur_dir");
  Alcotest.(check bool)
    "fmt not carried" false
  ...
  | Ok (Cached_failure f) ->
      Alcotest.(check (option string))
        "failure re-stamped" (Some "new-key") f.cache_key
  ...
  (match
     Incremental_solver.save
       Fpath.(cur_dir / "astring.0.8.5.json")
       (mk_solution ~cache_key:"stale-key" "astring.0.8.5")
   with
  | Ok () -> ()
  | Error (`Msg e) -> Alcotest.fail e);
  let reused2 =
    Incremental_solver.reuse_solutions ~expected_cache_key:"prev-key"
      ~rekey_to:"new-key" ~solutions_cache_dir:cur_dir ~previous_dir:prev_dir
      ~changed_packages:changed ~packages:[ "astring.0.8.5" ] ()
  in
  Alcotest.(check int) "overwrote stale entry" 1 reused2;
  match Incremental_solver.load Fpath.(cur_dir / "astring.0.8.5.json") with
  | Ok (Cached_solution s) ->
      Alcotest.(check (option string))
        "stale key replaced" (Some "new-key") s.cache_key
...
let test_reuse_tool_stems () =
  ...
  ignore (Bos.OS.Dir.create cur_dir);
  let key_prev =
    Incremental_solver.tool_cache_key ~repos:[ ("repo", "aaaa") ]
  in
  let key_cur = Incremental_solver.tool_cache_key ~repos:[ ("repo", "bbbb") ] in
  Alcotest.(check bool) "keys differ per repo state" true (key_prev <> key_cur);
  let solution =
    OpamPackage.Map.singleton (pkg "odoc.3.1.0") OpamPackage.Set.empty
  in
  let entry =
    Incremental_solver.Cached_solution
      {
        ...
        result =
          {
            Day11_solution.Solve_result.packages =
              OpamPackage.Set.singleton (pkg "odoc.3.1.0");
            build_deps = solution;
            doc_deps = solution;
            examined =
              OpamPackage.Name.Set.of_list
  ...
  in
  let reused =
    Incremental_solver.reuse_solutions ~expected_cache_key:key_prev
      ~rekey_to:key_cur ~solutions_cache_dir:cur_dir ~previous_dir:prev_dir
      ~changed_packages:changed ~packages:[ stem ] ()
  in
  Alcotest.(check int) "1 reused" 1 reused;
  match Incremental_solver.load Fpath.(cur_dir / (stem ^ ".json")) with
  | Ok (Cached_solution s) ->
      Alcotest.(check string)
        "package from body" "odoc.3.1.0"
        (OpamPackage.to_string s.package);
    ...
    [
      ...
      ( "Incremental_solver",
        [
          ...
          Alcotest.test_case "reuse tool stems" `Quick test_reuse_tool_stems;
