...
let show_progress ~os ~i ~n ~old ~deleted ~delete =
  let msg =
    if delete then
      Printf.sprintf "[gc] %s: %d/%d scanned, %d old, %d deleted" os i n old
        deleted
    else Printf.sprintf "[gc] %s: %d/%d scanned, %d old" os i n old
  in
      ...
      List.iteri
        (fun idx name ->
         ...
         with neither sentinel nor metadata scores 0.0. *)
          let last_used =
            match Day11_layer.Last_used.effective env layer_dir with
            | Some t -> t
            | None -> 0.0
          in
          if last_used < cutoff then (
           ...
           One small JSON read, and only for layers we'd actually free. *)
            let size =
              match
                Day11_layer.Meta.load env Fpath.(layer_dir / "layer.json")
              with
...
let du_term =
  let doc =
    "Also report total cache size via [du -sh] before and after (slow: \
     recursively walks the whole cache, including the html epoch trees). Off \
     by default."
  in
...
let cmd =
  ...
  let term =
    Term.(
      const run $ Common.profile_dir_term $ before_term $ delete_term $ du_term)
  in
  Cmd.v info term
