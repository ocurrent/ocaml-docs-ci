(** verify-solver: differential test of incremental solution reuse.
    ...
    Walks a series of opam-repository commits (oldest first). At each
    commit it solves every target twice:

    - {b full}: from scratch, no reuse — the reference arm;
    - {b incr}: seeded from the previous commit's incr solutions via
      {!Day11_batch.Incremental_solver.reuse_solutions} with the same
      diff + [expected_cache_key]/[rekey_to] discipline the
      ocaml-docs-ci solver op uses, then solving only what wasn't
      reused.

    Every target's [Solve_result] must then be identical across the
    two arms (JSON equality — packages, build_deps, doc_deps {e and}
    examined). At the first commit both arms solve from scratch, which
    doubles as a solver-determinism baseline: if full-vs-full already
    differs, incremental reuse can't be validated.
...
let commits_of_repo ~repo ~head ~count =
  let cmd = Printf.sprintf
    "git -C %s log --first-parent -n %d --format=%%H %s"
    (Filename.quote repo) count (Filename.quote head) in
  let ic = Unix.open_process_in cmd in
  let commits = ref [] in
  (try while true do commits := input_line ic :: !commits
...
let latest_version git_packages name =
  ...
  let versions =
    if OpamPackage.Version.Map.is_empty non_avoided
...
(* Solve [targets] not already present in [dir] at [sha], saving each
   ...
   (solved, failed). *)
let solve_missing ~sw env ~np ~ocaml_version ~repo ~sha ~dir ~cache_key
    targets =
  ignore (Bos.OS.Dir.create ~path:true dir);
  let need = List.filter (fun t ->
    not (Sys.file_exists
           (Fpath.to_string
              Fpath.(dir / (OpamPackage.to_string t ^ ".json")))))
    targets in
  if need = [] then (0, 0)
  else begin
    let results =
      Day11_solver_pool.Solver_pool.solve_many ~sw env
        ?ocaml_version ~np ~repos:[ (repo, sha) ] need in
    let failed = ref 0 in
    List.iter (fun (target, result) ->
      let path =
        Fpath.(dir / (OpamPackage.to_string target ^ ".json")) in
...
let compare_target ~full_dir ~incr_dir target =
  ...
  | Ok (Inc.Cached_solution a), Ok (Inc.Cached_solution b) ->
    ...
    if ja = jb then Same
      ...
      let fields = List.filter_map (fun (label, proj) ->
        if proj a.result <> proj b.result then Some label else None)
        [ "packages", (fun (r : SR.t) ->
            `List (List.map (fun p -> `String (OpamPackage.to_string p))
                     (OpamPackage.Set.elements r.packages)));
          "build_deps", (fun (r : SR.t) ->
            SR.to_json { r with doc_deps = OpamPackage.Map.empty;
                                examined = OpamPackage.Name.Set.empty });
          "doc_deps", (fun (r : SR.t) ->
            SR.to_json { r with build_deps = OpamPackage.Map.empty;
                                examined = OpamPackage.Name.Set.empty });
          "examined", (fun (r : SR.t) ->
            `List (List.map (fun n ->
                `String (OpamPackage.Name.to_string n))
...
let run repo head count shas np ocaml_version_str only work_dir keep =
  ...
  let commits = match shas with
    | [] -> commits_of_repo ~repo ~head ~count
    | l -> l
  in
  if List.length commits < 2 then begin
    Printf.eprintf
      "Need at least 2 commits to verify incremental reuse (got %d).\n"
      (List.length commits);
    exit 2
  end;
  let work_dir = match work_dir with
    | Some d -> Fpath.v d
    | None ->
      Fpath.v (Filename.get_temp_dir_name ())
      |> fun t -> Fpath.(t / Printf.sprintf "day11-verify-%d" (Unix.getpid ()))
  in
  ...
  let store, _head =
    Day11_opam.Git_utils.get_git_repo_store_and_hash repo in
  ...
  List.iteri (fun i sha ->
    ...
    Printf.printf "── commit %d/%d %s (%d targets) ──\n%!"
      (i + 1) (List.length commits) (short sha) (List.length targets);
    let full_dir = Fpath.(work_dir / "full" / short sha) in
    let incr_dir = Fpath.(work_dir / "incr" / short sha) in
    (* Full arm: always from scratch. *)
    let (n_full, f_full), t_full = time (fun () ->
      solve_missing ~sw env ~np ~ocaml_version ~repo ~sha
        ~dir:full_dir ~cache_key:(key_for sha) targets) in
    ...
    let reused =
      ...
      | Some (prev_sha, prev_incr_dir) ->
        ...
        let changed =
          let d1 = Day11_opam.Git_packages.diff_packages
              ~store prev_hash hash in
          let d2 = Day11_opam.Git_packages.diff_packages
              ~store hash prev_hash in
        ...
        let reused = Inc.reuse_solutions
            ~expected_cache_key:(key_for prev_sha)
            ~rekey_to:(key_for sha)
            ~solutions_cache_dir:incr_dir ~previous_dir:prev_incr_dir
            ~changed_packages:changed ~packages:target_strs () in
        Printf.printf "  changed names: %d; reused %d/%d\n%!"
          (OpamPackage.Name.Set.cardinal changed) reused
          (List.length targets);
        reused
    in
    total_reused := !total_reused + reused;
    let (n_incr, f_incr), t_incr = time (fun () ->
      solve_missing ~sw env ~np ~ocaml_version ~repo ~sha
        ~dir:incr_dir ~cache_key:(key_for sha) targets) in
    ...
    List.iteri (fun j (target, why) ->
      if j < 20 then
        Printf.printf "    MISMATCH %s: %s\n%!"
          (OpamPackage.to_string target) why)
      (List.rev !bad);
  ...
  if not keep then
    ignore (Bos.OS.Dir.delete ~recurse:true work_dir)
  else
    Printf.printf "  solutions kept in %s\n%!" (Fpath.to_string work_dir);
...
let count_term =
  let doc = "Number of first-parent commits to verify (newest of these \
             is $(b,--head); processed oldest first)." in
  Arg.(value & opt int 5 & info [ "count"; "n" ] ~docv:"N" ~doc)

let shas_term =
  let doc = "Explicit commit SHAs to verify, oldest first (overrides \
             $(b,--count)/$(b,--head))." in
...
let packages_term =
  let doc = "Restrict to these package names (repeatable). Default: \
             every package name at each commit, latest version." in
  Arg.(value & opt_all string [] & info [ "package"; "p" ] ~docv:"NAME" ~doc)

let work_dir_term =
  let doc = "Directory for the solution trees (default: a fresh dir \
             under \\$TMPDIR, deleted on success unless $(b,--keep))." in
...
let cmd =
  ...
  let info = Cmd.info "verify-solver" ~doc ~man:[
    `S Manpage.s_description;
    `P "Walks a series of opam-repository commits. At each commit every \
        target is solved twice: from scratch (the reference), and \
        incrementally (reusing the previous commit's solutions whose \
        examined set is untouched by the commits' changed packages — \
        the same machinery ocaml-docs-ci uses). The two results must \
        be identical for every target.";
    `P "The first commit solves from scratch in both arms, which \
        doubles as a solver-determinism baseline.";
    `P "Exits 1 if any mismatch is found.";
  ] in
  Cmd.v info
    Term.(const run $ repo_term $ head_term $ count_term $ shas_term
          $ np_term $ ocaml_version_term $ packages_term
          $ work_dir_term $ keep_term)
