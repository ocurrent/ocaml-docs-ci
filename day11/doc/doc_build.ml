...
(** Create tool binary mounts from driver and odoc tools.
    ...
    [Error names] when a binary can't be found in the tool layers — which in
    practice means the layer dir has been reclaimed out from under a cache that
    still records it as built. Returning it rather than raising matters: a raise
    escapes the dispatch callbacks in {!Day11_doc.Generate}, so the node's
    failure never reaches [history.jsonl] or [layer_status.jsonl] and the web UI
    goes on showing the package's last {e build} as its status. *)
let make_tool_mounts env ~os_dir ~(driver_tool : Day11_opam_layer.Tool.t)
  ...
  let wanted =
    [
      ...
      ("sherlodoc", driver_tool.builds, "/home/opam/doc-tools/bin/sherlodoc");
      (* [ocamlobjinfo] is what voodoo invokes via [Eio.Process] to
         ...
         the uniform fix. *)
      ("ocamlobjinfo", odoc_tool.builds, "/home/opam/doc-tools/bin/ocamlobjinfo");
    ]
  in
  ...
  let mounts =
    List.filter_map
      (fun (name, builds, dst) ->
        Option.map (fun src -> mount src dst) (find name builds))
      wanted
  in
  ...
  | names ->
      Error
        (Printf.sprintf
           "doc tool binaries not found in the tool layers (%s) — the tool \
            layers are missing or incomplete on disk"
           (String.concat ", " names))
...
(** Set up the prep structure and mounts for a doc container. Returns
    [Ok (universe, mounts, prep_dir)], or [Error] when the tool layers can't
    supply the doc binaries (see {!make_tool_mounts}) or on a hard
...
let prepare env ~(config : doc_config) ~build_layer ~universe pkg =
  ...
  match
    make_tool_mounts env ~os_dir:config.os_dir ~driver_tool:config.driver_tool
  ...
  | Ok tool_mounts -> (
      ...
      match
        Prep.create_with_mounts ~source_layer_dir:build_layer
          ~dest_layer_dir:prep_dir ~universe ~pkg ~installed_libs
          ~installed_docs
      with
