...
(** [compile env benv ~config ~build_layer ~universe ~build_deps_layers
    ...
    [universe] is the doc-deps universe id ([Universe.of_deps] of the transitive
    doc_deps closure); it namespaces the prep tree so the same build layer
    compiled under different universes yields distinct outputs. Reads source
    [.cmti]/[.ml] files from [build_layer], stacks [build_deps_layers] (for
    [ocamlobjinfo] and other build tools) and [dep_compile_layers] (for
...
(** [is_ocaml_package node] returns true if [node]'s {e transitive} build
    closure includes the virtual package [ocaml], OR [node] is one of the
    {!concrete_compiler_names} (which {e provides} [ocaml]). The second disjunct
    ensures the compiler's stdlib is documented; without it, every package's
    [Stdlib.*] xref in the rendered HTML lands as [xref-unresolved]. The closure
    (not just direct deps) is walked so packages that reach [ocaml] only through
    a dependency — e.g. [dune-rpc]/[dune-rpc-lwt] via [lwt]/[stdune], whose own
    opam files don't list [ocaml] — are still documented. [conf-*] system
    packages, [ocaml-options-*], and [ocaml-config] don't reach [ocaml] even
    transitively and so are still skipped. Unlike {!has_documentable_libs}, this
    works without the layer being built — useful for the first run on a cold
    cache. *)
