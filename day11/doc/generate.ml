...
let run_doc_node ~sw env benv ~os_dir ~html_dir ~driver_tool (dn : doc_node) =
  ...
  | Some (odoc_tool : Tool.t) -> (
      ...
      if dn.kind = Link && link_compile = None then
        ...
        let pkg = node.pkg
        and hash = dn.layer.hash
        and universe = dn.universe in
        let label =
          match dn.kind with
          | Compile -> "compile"
          | Doc_all -> "doc-all"
        ...
        let config : Doc_build.doc_config =
          { driver_tool; odoc_tool; os_dir; blessed = dn.blessed }
        in
...
let build_internal_plan ~os_dir:_ ~cache ~base_hash ~(driver_tool : Tool.t)
  ...
  in
  let rec visit ~(g : doc_graph) pkg : string * doc_node option * doc_node list
      =
  ...
  and visit_uncached ~(g : doc_graph) pkg :
    ...
    | Some (n : build) -> (
        let u =
          Day11_solution.Universe.of_deps (deps_or_empty g.g_trans_doc pkg)
        in
        ...
        | None ->
            ...
            let split =
              not
                (OpamPackage.Set.equal
                   (deps_or_empty g.g_build pkg)
                   (deps_or_empty g.g_doc pkg))
            in
           ...
           The link hash below feeds on [dn.layer.hash], so it cascades. *)
            ...
            let doc_node =
              match
                if Doc_build.is_ocaml_package n then g.g_compiler else None
              with
              | None -> None
              | Some compiler -> (
                  match
                    ( odoc_tool_of_compiler compiler,
                      odoc_final_of_compiler compiler )
                  with
                  | None, _ | _, None -> None
                      ...
                      let layer : build =
                        {
                          ...
                          pkg = n.pkg;
                          deps =
                            [ n; driver_final; odoc_final ]
                            @ List.map
                                (fun (dn : doc_node) -> dn.layer)
                                doc_deps;
            ...
            in
            let surfaced =
              match doc_node with Some dn -> [ dn ] | None -> doc_deps
            in
      ...
      let resolve pkg =
        ...
        | None ->
            let r =
              ...
              if Hashtbl.mem build_by_hash bh then
                Some
                  ( bh,
          ...
          | Some (bh, u_s) ->
              OpamPackage.Set.iter
                (fun dep ->
                  ...
                  | Some (dep_bh, dep_u_s) ->
                      add_dep_edge ~bh ~u_s ~dep_bh ~dep_u_s
                  | None -> ())
                direct)
        result.doc_deps;
      let g =
          ...
          (fun acc (m : build) ->
            ...
            OpamPackage.Map.add m.pkg
              (OpamPackage.Set.of_list
                 (List.map (fun (d : build) -> d.pkg) m.deps))
              acc)
          OpamPackage.Map.empty tool_builds
      in
      let g =
        {
          g_node =
            (fun pkg -> Hashtbl.find_opt by_pkg (OpamPackage.to_string pkg));
          g_build = build_graph;
          g_doc = build_graph;
  ...
  let compile_docall_doc_nodes =
    Hashtbl.fold
      (fun _ (_, dn, _) acc ->
        match dn with Some dn -> dn :: acc | None -> acc)
      memo []
  in
      ...
      (fun (dn : doc_node) ->
        ...
        | Compile, Some odoc_tool ->
           ...
           overlayfs rejects duplicate lowerdirs). *)
            let seen : (string, string * string) Hashtbl.t =
              Hashtbl.create 16
            in
            ...
            let dep_hashes =
              List.sort String.compare
                (List.map
                   (fun (ddn : doc_node) -> ddn.layer.hash)
                   dep_doc_nodes)
            in
            let link_hash =
              Day11_layer.Hash.of_strings
            ...
            let link_layer : build =
              {
                hash = link_hash;
                pkg = dn.build_node.pkg;
                deps =
                  [ dn.build_node; dn.layer ]
            ...
            Some
              {
                build_node = dn.build_node;
                kind = Link;
                layer = link_layer;
                doc_deps = dep_doc_nodes;
                compile_layer = Some dn.layer;
                compiler = dn.compiler;
                odoc_tool = dn.odoc_tool;
  ...
  let layers_of_kind k =
    List.filter_map
      (fun (dn : doc_node) -> if dn.kind = k then Some dn.layer else None)
      compile_docall_doc_nodes
  in
  let compile_list = layers_of_kind Compile in
  let doc_all_list = layers_of_kind Doc_all in
  Printf.printf
    "  plan: %d build, %d tool, %d compile, %d doc-all, %d link (solutions \
     %.1fs, tools %.1fs, links %.1fs)\n\
     %!"
    (List.length nodes) (List.length tool_nodes) (List.length compile_list)
    (List.length doc_all_list)
        ...
        (fun pkg nodes ->
          ...
          match
            Bos.OS.File.write path
              (Yojson.Safe.to_string (`List (List.rev nodes)))
          with
          | Ok () -> ()
          | Error (`Msg m) ->
...
let run ~sw env benv ~np ~os_dir ~html_dir ~cache ~base_hash
    ~(driver_tool : Tool.t) ~odoc_tools ~tool_source_dirs ~mounts ~run_log
    ~build_one ?(on_pkg_complete = fun _ ~cached:_ ~success:_ -> ())
    ?(on_doc_complete = fun _ ~cached:_ ~blessed:_ ~universe:_ ~success:_ -> ())
    ?snapshot_dir ~nodes ~solutions ~blessing_maps:_ () =
  let plan =
    build_internal_plan ~os_dir ~cache ~base_hash ~driver_tool ~odoc_tools
      ~nodes ~solutions
  in
    ...
    ~on_complete:(fun ~stats ~cached node success ->
      ...
      | Doc_all | Link ->
          on_doc_complete node ~cached
            ...
            ~universe:(node_universe_of_plan plan node)
            ~success
      | Build | Tool | Compile -> ());
      ...
      let find_result =
        Day11_sys.Run.run ~sw env
          Bos.Cmd.(
            v "sh"
            % "-c"
            % Printf.sprintf "find %s -name '*.html' -type f | wc -l"
                (Filename.quote (Fpath.to_string html_root)))
          None
      in
      try int_of_string (String.trim find_result.output) with _ -> 0
    else 0
  in
...
(** Resolve tools (driver + per-compiler odoc). Returns the tools and source
    ...
    When [snapshot_dir] is given (and the profile has no [odoc_repo] pin —
    pinned checkouts aren't captured by repo SHAs), each tool's [Solve_result]
    is cached at [<snapshot_dir>/tool_solutions/<pkg>@<compiler>.json] via
    {!Day11_batch.Incremental_solver}, keyed by the repo-set digest. A hit skips
    the ~13s solver-worker subprocess whose cost is almost entirely re-parsing
    the tool's dependency cone from the git store. Entries carry [examined] like
    package solutions, so ocaml-docs-ci's cross-snapshot reuse pass carries them
    over commits that don't touch the tool cone. *)
  ...
  if compiler_versions = [] then (
    ...
    let tool_cache_dir =
      ...
      | Some d when all_pin_dirs = [] ->
          let dir =
            Fpath.(d / Day11_batch.Incremental_solver.tool_solutions_dirname)
          in
    ...
    let solve_tool_cached ~pin_dirs ~source_dirs ?ocaml_version pkg =
      ...
      match cached with
      | Some result ->
          Day11_opam_build.Tools.plan_tool_of_result benv ~packages ~source_dirs
            ~cache pkg result
      | None -> (
          match
            Day11_opam_build.Tools.solve_tool ~sw env ~repos ~pin_dirs
              ~doc:false ?ocaml_version pkg
          with
          | Error _ as e -> e
          | Ok result ->
              (match tool_cache_dir with
              | Some dir ->
                  ignore
                    (Day11_batch.Incremental_solver.save (entry_file dir)
    ...
    let results =
      Eio.Fiber.List.map ~max_fibers:(List.length tasks)
        (function
          | `Driver ->
              let r =
                solve_tool_cached ~pin_dirs:[]
                  ~source_dirs:OpamPackage.Name.Map.empty
                  ?ocaml_version:driver_compiler driver_pkg
              in
              (`Driver, r)
          | `Odoc compiler_v ->
              let r =
                solve_tool_cached ~pin_dirs:all_pin_dirs
                  ~source_dirs:all_source_dirs ~ocaml_version:compiler_v
                  odoc_pkg
              in
              (`Odoc compiler_v, r))
        tasks
...
let doc_format_versions ~(driver_tool : Tool.t) ~odoc_tools =
  let all_builds =
    driver_tool.builds
    @ List.concat_map (fun (_, (t : Tool.t)) -> t.builds) odoc_tools
  in
  List.filter_map
    (fun (b : build) ->
      if
...
let plan_doc_dag ~sw env (ctx : Day11_batch.Profile_ctx.t) ~mounts ~build_one
  ...
  match
    resolve_tools ~sw env ctx.benv ~packages:ctx.git_packages
      ...
      ~cache:ctx.hash_cache ?driver_compiler:ctx.driver_compiler ?snapshot_dir
      ~solutions ()
  with
  | None -> None
  | Some (driver_tool, odoc_tools, all_source_dirs) ->
      ...
      let plan =
        build_internal_plan ~os_dir:ctx.os_dir ~cache:ctx.hash_cache
          ~base_hash:ctx.base.hash ~driver_tool ~odoc_tools ~nodes ~solutions
      in
      let dispatch =
     ...
     cascade attribution is derivable from the DAG, not stored. *)
      ...
      write_universes_if_requested ~snapshot_dir plan;
      Printf.printf
        "  plan writes (dag.json, plan.json×pkgs, universes): %.1fs\n%!"
...
let build_tools_and_run ~sw env (ctx : Day11_batch.Profile_ctx.t) ~np ~mounts
    ~build_one ?(on_pkg_complete = fun _ ~cached:_ ~success:_ -> ())
    ?(on_doc_complete = fun _ ~cached:_ ~blessed:_ ~universe:_ ~success:_ -> ())
    ?snapshot_dir ~run_log ~nodes ~solutions ~blessing_maps:_ () =
  let html_dir =
    match ctx.profile.html_dir with
    | Some d -> Fpath.v d
    | None -> Fpath.(ctx.os_dir / "html")
  ...
  | Some (driver_tool, odoc_tools, all_source_dirs) ->
      ...
      let doc_count, doc_html =
        run ~sw env ctx.benv ~np ~os_dir:ctx.os_dir ~html_dir
          ~cache:ctx.hash_cache ~base_hash:ctx.base.hash ~driver_tool
