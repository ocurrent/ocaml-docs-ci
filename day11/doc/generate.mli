...
(** {1 Internal planning — exposed for white-box testing}
    ...
    [build_internal_plan] is the pure core of doc-DAG construction: given the
    build nodes and solutions (plus the resolved doc tools) it derives the
    per-universe compile/doc-all/link nodes, their layer hashes, and each node's
    blessing. It runs no containers, so it is unit-testable on any platform —
    see [test/test_generate_plan.ml]. *)
...
type internal_plan = {
  ...
  meta : (string, doc_node) Hashtbl.t;
      (** Keyed by each node's own layer hash; the single source of truth for a
          node's kind, universe and blessing. *)
  driver_tool : Day11_opam_layer.Tool.t;
}
