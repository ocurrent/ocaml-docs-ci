...
(* [voodoo-prep] used to copy the switch's whole [doc/] tree, which is how
   ...
   [doc/<pkg>/<file>]. *)
let test_prep_copies_doc_files () =
  with_tmp_dir @@ fun dir ->
  let source = Fpath.(dir / "build-layer") in
  let dest = Fpath.(dir / "doc-layer") in
  let doc_dir =
    Fpath.(
      source / "fs" / "home" / "opam" / ".opam" / "default" / "doc" / "astring")
  in
  ...
  let copied rel =
    Alcotest.(check bool)
      ("copied " ^ rel) true
      (Bos.OS.File.exists Fpath.(pkg_doc // v rel) |> Result.get_ok)
  in
  ...
  Alcotest.(check string)
    "README contents" "# astring"
    (Bos.OS.File.read Fpath.(pkg_doc / "astring" / "README.md") |> Result.get_ok);
  (* the package is discoverable via its own doc files, so no stub needed *)
  Alcotest.(check bool)
    "no stub index.mld" false
