...
(** [effective env layer_dir] is the best available "last used" time for
    eviction decisions: the sentinel mtime when there is one, else the layer's
    own [layer.json] mtime.

    The fallback matters because {!touch} is only called when a layer is
    {e re-used} (a cache hit, or stacked as an overlay lower). A layer that was
    built and then only ever bind-mounted — the odoc / odoc-driver tool layers
    are the case that bit us — has no sentinel however fresh it is, and treating
    that as "epoch 0" makes an LRU sweep delete it immediately. Deleting a tool
    layer that the OCurrent cache still records as built strands every doc node
    behind it (see [Day11_prep.reconcile_cache] in ocaml-docs-ci).

    [None] only when the layer has neither sentinel nor [layer.json] — residue
    from a failed attempt, which stays freely evictable. *)
