...
(** [compute ~inputs] is the epoch hash for a doc toolchain: a digest of
    ...
    *versions* ([name.version]) of the doc-format-determining tool packages —
    odoc, odoc-driver (voodoo), odoc-md, sherlodoc, odig. We key on versions,
    not the tools' content-addressed build hashes, so a deep transitive dep bump
    in opam-repository no longer churns the epoch (and forces a full re-link of
    the world); only a change to a tool that actually shapes the HTML does.
    Master/overlay builds are still captured — they carry the git SHA in the
    version string. Per-package inputs are intentionally excluded — they update
...
(** [to_gc ~base_dir ~keep] is the epoch directories that {!gc} would remove —
    the ones beyond the [keep] most-recent, never including the currently-live
    epoch. Pure and fast (readdir + stat); the caller does the actual deletion,
    which can be very slow and must not run on a latency-sensitive event loop.
*)

val gc : base_dir:Fpath.t -> keep:int -> int
(** [gc ~base_dir ~keep] removes old epoch directories, keeping the [keep] most
    recent. Returns the number deleted. Note: deletes in-process and
    synchronously — fine for CLI/tests, but callers on an event loop should use
    {!to_gc} and delete out-of-band. *)
