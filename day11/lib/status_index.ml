...
type node_outcome = {
  ...
  ok : bool;
  cascaded : bool;
      (* only meaningful when [not ok]: the node never ran
...
let of_outcomes ~run_id ~scanned (outcomes : node_outcome list) : t =
  ...
  Log.app (fun f ->
      f "generated status (run %s): %d packages, blessed=%d non_blessed=%d"
...
let final_status_of_outcomes (items : (string * node_outcome) list) :
  ...
  Hashtbl.fold
    (fun pkg os acc ->
      ...
      let status =
        if any (fun o -> (not o.ok) && (not o.cascaded) && not o.is_doc) then
          "build_failure"
