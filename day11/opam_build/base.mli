(** Base image management.
    ...
    from. The base contains the profile's OS image (e.g. debian or ubuntu) with
    opam, build tools, and pre-initialised opam repositories. Docker is used to
    produce the image, which is then imported as a layer for overlayfs use.

    The base layer lives under its os_dir ([<cache>/<os_dir_name>/base]), so
    different OS variants never share — and silently overwrite — one directory.
...
(** Ensure a base layer exists for the given Docker [image] tag under [os_dir],
