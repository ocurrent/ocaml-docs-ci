...
(** Default container env for opam-build containers.
    ...
    NB on Eio programs run in these containers (odoc_driver_voodoo in the doc
    stages): at very high container concurrency (~480) the default io_uring
    backend can fail at startup with ENOMEM from io_uring_queue_init — a fast,
    retryable failure. Forcing EIO_BACKEND=posix avoids that but trades it for a
    rare missed- SIGCHLD hang in eio_posix's child reaping that wedges executor
    slots permanently — strictly worse. So: stay on io_uring and keep container
...
(** Build an OCI spec template for an opam-build container running [cmd] via
    ...
    [?jobs] additionally exports [OPAMJOBS], capping opam's [%{jobs}%] variable.
    The cpuset alone is not enough: opam derives its default jobs from the
    *online* CPU count (sysconf), which ignores cpuset affinity — so a build
    pinned to 4 cpus would still run e.g. [dune -j 767] on a 768-core host,
    multiplying its memory footprint by the spawned-compiler count. *)
let opam_build_spec ?cpuset ?numa_mems ?jobs ~cmd ~mounts ~uid ~gid () :
  ...
  in
  Day11_container.Oci_spec.make ~cwd:"/home/opam" ~hostname:"builder" ~env
    ~mounts ~network:true ?cpuset ?numa_mems
...
(** Collect the transitive dep layer hashes of a build node, in the order they
    ...
    iteration order: two runs of the same node could stack the layers
...
let build ~sw env (benv : Types.build_env) ~opam_repositories ?(mounts = [])
  ...
  let spec =
    opam_build_spec ?cpuset ?numa_mems ?jobs ~cmd:strategy.Types.cmd
  ...
  | Ok (run, upper, timing) ->
      ...
      let () =
        match
          Day11_sys.Sudo.run ~sw env
            Bos.Cmd.(
              ...
              % "-c"
              % Printf.sprintf
                  "set -o pipefail; getfattr -h -R -m trusted.overlay.opaque \
                   --absolute-names %s | awk '/^# file:/ {print $3}' | xargs \
                   -r -I{} setfattr -x trusted.overlay.opaque {}"
        ...
        | Error (`Msg m) ->
            Log.err (fun f ->
                f
                  "opaque-xattr strip failed for %a (is the 'attr' package \
                   installed?): %s"
                  Fpath.pp target_fs m)
      in
      let _ =
        Day11_sys.Sudo.run ~sw env
