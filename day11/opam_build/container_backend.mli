...
(** Build an OCI spec template for a container that runs [cmd] via [bash -c],
    ...
    @param numa_mems
      ...
      Pairs with [?cpuset].
    @param jobs
      Export [OPAMJOBS=jobs] so opam's [%{jobs}%] matches the cpuset size.
      opam's default comes from the online CPU count, which ignores cpuset
      affinity — without this a pinned build still spawns host-cpu-count
      compiler processes. *)
...
val collect_transitive_dep_pkgs : Day11_opam_layer.Build.t -> OpamPackage.t list
(** Transitive dependency packages of a build node (the full closure, deduped,
    in overlay-stack order). The package-level view of
