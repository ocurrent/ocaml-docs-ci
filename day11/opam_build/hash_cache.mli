...
(** [create ~find_opam ?patches ()] creates a new hash cache. When [patches] is
    ...
    When [find_oid] (package -> its version-dir tree OID at the current repo
    state) is provided, effective-part digests are served from a process-global
    cache keyed by package and validated by the OID — unchanged packages cost no
    opam read or parse even across Profile_ctx reloads; only versions whose tree
    OID moved are re-parsed. The cache is keyed by package identity, never by
    bare OID: byte-identical twin package dirs (templated multi-package
    releases, re-releases) share a tree OID but not a digest — the digest
    includes the name/version stamped in at parse time. *)
