...
let plan_tool_of_result (benv : Types.build_env) ~packages
  ...
  Log.info (fun m -> m "Tool %s: %d nodes in DAG" pkg_str (List.length nodes));
...
let solve_tool ~sw env ~repos ?(constraints = []) ?(pin_dirs = []) ?(doc = true)
...
let plan_tool ~sw env (benv : Types.build_env) ~packages ~repos
    ...
    ?(source_dirs = OpamPackage.Name.Map.empty) ?cache target =
  match
    solve_tool ~sw env ~repos ~constraints ~pin_dirs ~doc ?ocaml_version target
  with
  | Error _ as e -> e
  | Ok result ->
