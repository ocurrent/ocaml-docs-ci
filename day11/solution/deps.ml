...
let transitive_deps deps =
  ...
  in
  OpamPackage.Map.iter (fun v _ ->
    if not (Hashtbl.mem index v) then connect v) deps;
  ...
  OpamPackage.Map.mapi
    (fun pkg _ -> Hashtbl.find scc_closure (Hashtbl.find scc_of pkg)) deps
