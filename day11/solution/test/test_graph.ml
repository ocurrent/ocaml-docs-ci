...
let test_transitive_deps_cycle () =
  ...
  Alcotest.(check bool)
    "cycle closure = {a,b,c,x}" true
    (OpamPackage.Set.equal (clo a) (OpamPackage.Set.of_list [ a; b; c; x ]));
    ...
    [
      ( "Deps",
        [
          ...
          Alcotest.test_case "transitive_deps cycle" `Quick
            test_transitive_deps_cycle;
