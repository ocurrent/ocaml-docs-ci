...
(** Read the [x-extra-doc-deps] extension field of a package's opam file. These
    ...
    [depends:] guarded by [{with-doc & post}]. {!augment_depends} below realises
    that equivalence by rewriting the opam file's [depends:] before it is handed
    to opam-0install (see {!candidates}), so the standard opam pipeline (solver
    \+ post-solve dep computation) handles them transitively without any other
    code path needing to consult the extension field directly. *)
...
(** Rewrite [opam]'s [depends:] with synthetic [{with-doc & post}] atoms for
    every entry in [x-extra-doc-deps]. The result is what the package would have
    ...
    This must rewrite the opam file itself (in {!candidates}) rather than
    augment formulas passing through {!filter_deps}: opam-0install routes both
    [depends:] and [conflicts:] through [filter_deps], so augmenting there also
    appends the extras to the conflicts formula — an unconstrained conflict atom
    excludes every version of the extra, making any package with
    [x-extra-doc-deps] (and no pre-existing [conflicts:] escape branch)
    unsolvable. *)
...
(** Drop dependency atoms that are [odoc] guarded by [{with-doc}] (the
    near-universal opam convention "odoc is needed to build my docs"). odoc is
    the doc TOOL — docs-ci mounts it from the prebuilt per-compiler tool layer,
    never from a package's own solution — so it should not be a solved
    dependency at all. Removing it at the formula level (before filtering) means
    odoc is never pulled into a package's universe, which in turn means odoc's
    own [x-extra-doc-deps] (odoc-driver / sherlodoc / odig → eio, js_of_ocaml,
    tyxml, …) never fire: the whole doc-toolchain closure stops leaking into
    every package's doc-deps. odoc kept when it's a *real* (unguarded) dep —
    e.g. odoc-driver genuinely depends on odoc — so only the with-doc convention
    is stripped. *)
let drop_odoc_with_doc (f : OpamTypes.filtered_formula) :
    ...
    OpamFormula.fold_left
      (fun acc foc ->
        ...
        | OpamTypes.Filter filt ->
            List.exists
              (fun v -> OpamVariable.Full.to_string v = "with-doc")
  ...
  OpamFormula.map
    (fun (name, cond) ->
      if OpamPackage.Name.equal name odoc && mentions_with_doc cond then
        OpamFormula.Empty
