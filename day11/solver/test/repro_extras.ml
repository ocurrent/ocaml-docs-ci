...
let () =
  List.iter
    (fun t ->
      ...
      (match
         Day11_solver.Solve.solve ~packages:Day11_opam.Git_packages.empty ~env
           ~pins ~doc:true target
       with
      | Ok r ->
          let sel =
            OpamPackage.Map.keys r.build_deps |> List.map OpamPackage.to_string
          in
          Printf.printf "  OK: %s\n" (String.concat " " sel);
          let docsel =
            OpamPackage.Map.keys r.doc_deps |> List.map OpamPackage.to_string
          in
