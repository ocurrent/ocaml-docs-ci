...
|}
  ...
  in
  match
    Solve.solve ~packages:Day11_opam.Git_packages.empty ~env ~pins ~doc:true
      (pkg "tgt.1")
  with
  | Error (diag, _) -> Alcotest.fail (Printf.sprintf "Solve failed: %s" diag)
  | Ok result ->
      let solved =
        OpamPackage.Set.fold
          (fun p acc -> OpamPackage.Name.to_string (OpamPackage.name p) :: acc)
          result.Day11_solution.Solve_result.packages []
      in
      List.iter
        (fun n ->
          Alcotest.(check bool) (n ^ " in solution") true (List.mem n solved))
        [ "tgt"; "extra1"; "extra2" ];
      ...
      Alcotest.(check bool)
        "extra1 not in build deps" false (mem "extra1" build_set)
