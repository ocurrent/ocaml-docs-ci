...
module Analysis = struct 
  ...
  type t = {
    lockfile: OpamParserType.opamfile;
    dev_repos_output: string list;
  } [@@deriving yojson]
  ...
  let build_project_list (packages: Opamfile.pkg list) dev_repos_output =
    ...
    List.iter (fun (pkg: Opamfile.pkg) -> match StringMap.find_opt pkg.repo !repo_map with 
...
module Analyse = struct
  ...
  let build () job { Key.repos; packages } =
    ...
    let spec =
      ...
      @ generate_monorepo
    in
    let dockerfile = Obuilder_spec.Docker.dockerfile_of_spec ~buildkit:true spec
    in
    Current.Job.log job "Starting docker build for analysis.";
    let>> id = Current.Process.with_tmpdir (fun tmpdir ->
    ...
    in
    let>> lockfile_str = 
      let cmd = Current_docker.Raw.Cmd.docker ~docker_context:None
        ["run"; "-i"; id; "cat"; "/src/monorepo.opam.locked"]
      in Current.Process.check_output ~cancellable:true ~job cmd 
    in
    let lockfile = OpamParser.string lockfile_str "monorepo.opam.locked" in
    let packages = Opamfile.get_packages lockfile in 
    let>> dev_repos_str =
      let cmd = Current_docker.Raw.Cmd.docker ~docker_context:None
        (["run"; "-i"; id; "opam"; "show"; "--field"; "name:,dev-repo:"] @ (List.map (fun (pkg: Opamfile.pkg) -> pkg.name) packages))
      in Current.Process.check_output ~cancellable:true ~job cmd 
    in
