(** Manual epoch garbage-collection as a [Dangerous]-level OCurrent output.
    ...
    symlink (instant), while reclaiming old epoch dirs is its own
    confirmable action. Each epoch dir can hold millions of files, so the
    deletion is shelled out to [rm -rf] via {!Lwt_process} — it runs as a
    subprocess and never blocks the OCurrent/Lwt event loop (an in-process
    [Bos.OS.Dir.delete] here would freeze the whole engine for the
    multi-minute duration of the delete).
...
module Op = struct
  ...
  let pp f ((k : Key.t), ()) =
    Fmt.pf f "gc epochs in %s" k.base_dir
  ...
  let publish No_context job (key : Key.t) () =
    ...
    match dirs with
    | [] ->
      Current.Job.log job "No old epochs to gc (keeping %d most-recent + live)" keep;
      Lwt.return (Ok ())
    | _ ->
      ...
      let rec loop = function
        ...
        | dir :: rest ->
          ...
          (* [rm -rf] in a subprocess: non-blocking for the event loop,
             ...
             millions of files. *)
          let* status = Lwt_process.exec ("", [| "rm"; "-rf"; "--"; path |]) in
          (match status with
           | Unix.WEXITED 0 -> loop rest
           | Unix.WEXITED n ->
             Lwt.return (Error (`Msg (Printf.sprintf
               "rm -rf %s exited %d" path n)))
           | Unix.WSIGNALED n | Unix.WSTOPPED n ->
             Lwt.return (Error (`Msg (Printf.sprintf
               "rm -rf %s killed by signal %d" path n))))
      in
      let* result = loop dirs in
      (match result with
       | Ok () ->
         Current.Job.log job "gc complete (%d epoch dir%s removed)"
           (List.length dirs) (if List.length dirs = 1 then "" else "s")
       | Error _ -> ());
      Lwt.return result
end
...
(** [gc ~base_dir ~epoch_hash] is a [Dangerous] OCurrent node that, once
    confirmed in the web UI, deletes old epoch dirs under [base_dir]
    (keeping the {!keep} most-recent plus the live one). [epoch_hash] only
    keys the node so a fresh confirmation appears per promotable epoch. *)
