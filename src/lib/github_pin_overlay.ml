...
module Op = struct
  ...
  let ensure_upstream_clone job ~url ~branch ~upstream =
    ...
    if Sys.file_exists git_dir then
      ...
      match branch with
      | Some b ->
          run_sh job "git clone -b %s %s %s" (Filename.quote b)
            (Filename.quote url) (Filename.quote p)
      | None ->
          run_sh job "git clone %s %s" (Filename.quote url) (Filename.quote p)
  ...
  let build () job (key : Key.t) =
    ...
    let result =
      let** () =
        ensure_upstream_clone job ~url:key.url ~branch:key.branch ~upstream
      in
      ...
      let label =
        ...
        | Some b ->
            String.map
              ...
              b
      in
      let version_str = Printf.sprintf "%s+%s.%s.%s" tag label epoch sha7 in
...
let spec_of_arg s =
  ...
  | None ->
      Error
        (`Msg
           (Printf.sprintf "--github-pin-overlay %S: expected URL[#BRANCH]=PATH"
              s))
  | Some i ->
      ...
      let url, branch =
        match String.rindex_opt url_part '#' with
        | Some j ->
            let b =
              String.sub url_part (j + 1) (String.length url_part - j - 1)
            in
      ...
      else Ok { url; branch; path = Fpath.v path }
