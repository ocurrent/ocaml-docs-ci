  ...
  |> Spec.add
       [
         ...
         copy ~from:(`Build "tools")
           ...
           ~dst:"/home/opam/";
         run
           "mv ~/odoc $(opam config var bin)/odoc && cp ~/voodoo-gen $(opam \
            config var bin)/voodoo-gen";
         ...
         (* Run voodoo-gen *)
         workdir
           (Fpath.to_string (Storage.Base.generation_folder `Linked generation));
         run
           "OCAMLRUNPARAM=b opam exec -- /home/opam/voodoo-gen -o %s -n %s \
            --pkg-version %s"
...
module Gen = struct
  ...
  let pp f Key.{ compile; _ } =
    Fmt.pf f "Voodoo gen %a" Package.pp (Compile.package compile)
  ...
  let build generation job (Key.{ compile; voodoo; config; base } as key) =
    ...
    let spec =
      spec ~ssh:(Config.ssh config) ~generation ~voodoo ~base ~blessed compile
    in
    ...
    with Invalid_argument _ ->
      Lwt.return_error (`Msg "Gen: failed to parse output")
end
...
let v ~generation ~config ~name ~voodoo compile =
  ...
  |> let> compile
     ...
     let output =
       GenCache.get generation Gen.Key.{ compile; voodoo; config; base }
     in
