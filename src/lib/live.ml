module Op = struct
  ...
  let publish ssh job name (generation, kind) =
    ...
    let live_file = Fpath.add_ext "history" live_folder in
    let command =
      Bos.Cmd.(
        v "ssh" % "-p"
        ...
        % Fmt.str "ln -sfT %a %a && echo '%a' >> %a" Fpath.pp target_folder Fpath.pp live_folder
            Fpath.pp new_generation_folder Fpath.pp live_file)
    in
