...
(** [repo_label] is a stable human-readable identifier for the repo
    ...
    The result pairs the tracked packages with the {b commit hash}
    they were read from. The underlying op is latched: right after
    the repo moves, the current still carries the previous commit's
    list while re-tracking. Consumers that combine it with other
    repo-derived inputs must verify the embedded commit matches
    their view of the repo and treat mismatches as "not ready yet",
    or they will compute one evaluation on torn inputs. *)
...
(** Union per-repo tracking results with "later repos override"
    ...
    Sorted by package (the order feeds cache-key digests). *)
