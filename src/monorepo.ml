...
module Assembler = struct 
  ...
  module Key = struct 
    ...
    let digest {projects; _} = List.map (fun (_,x) -> Current_git.Commit.hash x) projects |> String.concat ";" 
  ...
  let build repos job {Key.monorepo; projects;} = 
    ...
    Current.Process.with_tmpdir (fun monorepo_path -> 
      List.map (fun (name, commit) -> (* setup monorepo *)
      ...
      |> Lwt.all >>= fun _ -> 
        Bos.OS.File.write (Fpath.(monorepo_path / "monorepo.opam")) (OpamPrinter.opamfile monorepo) |> or_raise;
...
let monorepo_main ~analysis ~repos () =
  let projects = 
    let* analysis = analysis in (* Bind: the list of tracked projects is dynamic *)
    let projects = Analyse.Analysis.projects analysis in
    Printf.printf "got %d projects to track.\n" (List.length projects);
    List.map (fun (x: Analyse.Analysis.project) -> 
      ...
      (x.name, commit)
    ) projects |> Current.list_seq
  in  
  Current.component "Monorepo assembler" |> 
  let> projects = projects
  and> analysis = analysis 
  and> repos = Current.list_seq repos 
  in
