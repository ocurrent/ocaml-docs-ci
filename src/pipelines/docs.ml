...
let v_for_profile ~config ~eio_env ~cache_dir:_ ?cpu_slots
  ...
  let solutions =
    Day11_solver.solve ~env ~np:(Config.jobs config) ~profile_name:profile.name
      ...
      ~pinned_versions ~cache_dir:ctx.cache_dir tracks
  in
  let* solutions in
  ...
  in
  Log.info (fun f ->
      f "[%s] %d build nodes (build_dag %.1fs)" profile.name (List.length nodes)
        (Unix.gettimeofday () -. t_dag));
  ...
  let on_doc_complete node ~blessed ~universe ~success =
    ...
    Day11_batch.Recorder.record_doc recorder node ~blessed ~universe ~success
  in
  ...
  (let n = Day11_prep.reconcile_plan ~env ~os_dir:ctx.os_dir all_dag_nodes in
   if n > 0 then
     Log.info (fun f ->
         f
           "[%s] cache reconcile: invalidated %d node(s) whose layer the disk \
            has no record of"
           profile.name n));
  ...
  Log.info (fun f ->
      let edges =
        List.fold_left
          ...
          0 all_dag_nodes
      in
      f "[%s] OCurrent graph: %d nodes, %d dep edges in %.1fs" profile.name
        (List.length all_dag_nodes)
        edges
  ...
  let builds =
    ...
    if List.compare_lengths results all_dag_nodes <> 0 then ()
      ...
      List.iter2
        (fun (n : Day11_opam_layer.Build.t) res ->
          Hashtbl.replace ok_of n.hash (Result.is_ok res))
        all_dag_nodes results;
      let dep_ok h =
        match Hashtbl.find_opt ok_of h with Some b -> b | None -> true
      in
      ...
      let node_rows =
        List.map2
          (fun (dag_node : Day11_opam_layer.Build.t) res ->
            let kind = node_kind dag_node in
            let is_doc =
              match kind with
              | Day11_doc.Generate.Build | Tool -> false
              | Compile | Doc_all | Link -> true
            in
      ...
      Metrics.set_status ~profile:profile.name
      ...
      let layer_counts =
        ...
        List.iter2
          (fun (dn : Day11_opam_layer.Build.t) res ->
            let side =
              match node_kind dn with
              | Day11_doc.Generate.Build -> "build"
              | Tool -> "tool"
      ...
      let has_blessed_doc =
        ...
        List.iter
          (fun (p, (o : Day11_lib.Status_index.node_outcome)) ->
            if o.blessed && o.is_doc then Hashtbl.replace t p ())
          pkg_outcomes;
        t
      in
      let blessed_doc_success =
        List.length (List.filter (fun (_, s) -> s = "doc_success") final)
      in
      let blessed_doc_failure =
        List.length
          (List.filter (fun (p, _) -> Hashtbl.mem has_blessed_doc p) final)
        - blessed_doc_success
      in
      ...
      let solver_failure =
        ...
        |> List.length
      in
      Metrics.set_packages ~profile:profile.name ~solver_failure
        ~not_documentable ~blessed_doc_success ~blessed_doc_failure;
