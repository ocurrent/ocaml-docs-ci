...
let compile ~(blessed: Package.Blessed.t Current.t) (preps: Prep.t list Current.t) =
  ...
  let* preps = preps in
  let pkg_preps = List.map (fun prep -> Prep.package prep, prep) preps |> List.to_seq |> Package.Map.of_seq
  in
  ...
  let rec get_compilation_job pkg =
    ...
    | None ->
      let deps =
        pkg |> Package.universe |> Package.Universe.deps |> List.filter_map get_compilation_job |> Current.list_seq
      in
      let result = Package.Map.find_opt pkg pkg_preps |> Option.map (fun prep -> prep |> Current.return |> Compile.v ~blessed ~deps) in
      jobs := Package.Map.add pkg result !jobs;
      result
  in
  List.filter_map (fun prep -> prep |> Prep.package |> get_compilation_job) preps |> Current.list_seq
...
let v ~opam () =
  ...
  in
  let all_packages = (* todo: add a append-only layer at this step *)
    all_packages_jobs
    |> Current.map (List.map Package.all_deps)
  ...
  let prepped = 
    ...
    @@ let+ res =
      ...
      |> List.flatten
  in
  let blessed = Current.map (fun prep -> prep |> List.map Prep.package |> Package.Blessed.v) prepped in
  let compiled = compile ~blessed prepped
  in
  Current.ignore_value compiled
  (*
