...
let job_db_path () =
  ...
  | None -> (
      let candidates =
        [ "/var/lib/ocurrent/var/db/sqlite.db"; "var/db/sqlite.db" ]
      in
...
(** The snapshot's own creation time: the ISO-8601 UTC timestamp from
    [repos.json] (written by [Snapshot.save]). Falls back to the directory
    mtime, formatted identically, only when repos.json is missing/unreadable. We
    deliberately do NOT use the directory mtime as the primary signal: it is
    bumped whenever any file inside is rewritten (e.g. the [pkgs_summary]
    regeneration on startup), which can float a long-finished snapshot above the
    live one — the cause of the dashboard featuring a stale "finished" snapshot
    as latest. *)
let snapshot_created dir =
  ...
  | Error _ -> (
      try
        let tm = Unix.gmtime (Unix.stat (Fpath.to_string dir)).Unix.st_mtime in
        Printf.sprintf "%04d-%02d-%02dT%02d:%02d:%02dZ" (tm.Unix.tm_year + 1900)
          (tm.Unix.tm_mon + 1) tm.Unix.tm_mday tm.Unix.tm_hour tm.Unix.tm_min
          tm.Unix.tm_sec
      with _ -> "")

(** List a profile's snapshots, newest first by creation time. ISO-8601 UTC
    timestamps sort lexicographically in chronological order, so a plain string
...
(** A snapshot is complete once its run finished and wrote [final_status.json]
    (see {!Day11_lib.Status_index.write_final_status}) — a run that is still in
    flight, or was superseded before every planned node resolved, never writes
    it. Package diffs are only offered between complete snapshots: an incomplete
    side has no authoritative per-package statuses to compare. *)
...
(** Find the key of the nearest {e completed} snapshot chronologically before
    [current_key] in this profile. Returns [None] if there is none. Used for the
    "Diff against previous" button on {!snapshot_detail} — diffs only work
    between completed snapshots, so incomplete ones are skipped over. *)
...
let snapshot_detail ~ctx name key =
  object
    ...
    method! private get web_ctx =
                    ...
                    [
                      txt
                        "Builds count compiled package layers; docs count \
                         compile+link (or doc-all) stages. Counts are per node \
                         (a package solved in N universes counts as N). \
                         'Blessed' means the node belongs to the chosen \
                         primary universe for its package; build nodes aren't \
                         universe-specific, so blessed builds is normally \
                         empty. A dependency_failure is a cascade — the node \
      ...
      let diff_link =
        match
          if snapshot_completed snapshot_dir then
            find_previous_snapshot_key ctx name key
          else None
        with
        | None -> []
        | Some prev ->
      ...
      let dag_data =
        timing "dag_data" (fun () ->
            match
              timing "read_dag_cached" (fun () -> read_dag_cached snapshot_dir)
            with
            | Error _ -> None
            | Ok entries ->
                let os_dir =
                ...
                let cascade_table =
                  ...
                  | Some d ->
                      let status_index =
                        timing "load_layer_status_cached" (fun () ->
                            load_layer_status_cached d)
                      in
        ...
        timing "failures_section" (fun () ->
            ...
            | Some (entries, cascade_table) -> (
                ...
                | _ ->
                    let by_kind =
                      List.fold_left
                        (fun acc (e : Day11_lib.Dag_marshal.entry) ->
                          let k = kind_label e.kind in
                          let n = try List.assoc k acc with Not_found -> 0 in
                          (k, n + 1) :: List.filter (fun (kk, _) -> kk <> k) acc)
                        [] failed
                    in
                    let by_kind =
                      List.sort (fun (_, a) (_, b) -> compare b a) by_kind
                    in
                    let summary =
                      String.concat ", "
                        (List.map
                    ...
                    let sorted =
                      List.sort
                        (fun (a : Day11_lib.Dag_marshal.entry) b ->
                          compare
                ...
                let failures, with_history =
                  List.fold_left
                    (fun (fs, wh) pkg_str ->
                      ...
                      | Some (status, _, _) when status = "success" ->
                          (fs, wh + 1)
                      | Some (status, category, build_hash) ->
                      ...
                      p
                        [
                          em
                            [
                              txt
                                "Snapshot still being prepared — no dag.json \
                        ...
                        [
                          em
                            [
                              txt
                                (Printf.sprintf
                                   "No dag.json and no history yet (%d/%d \
                                    packages)."
        ...
        timing "overview+cascade" (fun () ->
            ...
            | Some (entries, cascade_table) ->
                    ...
                    (fun k ->
                      let cells =
                        List.map
                          (fun b ->
                            let n =
                              try Hashtbl.find counts (k, b)
                              with Not_found -> 0
                            in
                            td [ txt (string_of_int n) ])
                          buckets
                      in
                      tr (th [ txt k ] :: cells))
                    kinds
                ...
                let total_for b =
                  List.fold_left
                    (fun acc k ->
                      acc + try Hashtbl.find counts (k, b) with Not_found -> 0)
                    0 kinds
                in
                      ...
                      p
                        [
                          em
                            [
                              txt
                                "Every planned node classified by on-disk \
                    ...
                    [
                      ...
                      p
                        [
                          txt
                            (Printf.sprintf
                               "All %d nodes finished — ok=%d, failed=%d, \
                                cascade=%d"
                               (ok + failed + cascade)
                               ok failed cascade);
                        ];
                ...
                List.iter
                  (fun (e : Day11_lib.Dag_marshal.entry) ->
                    Hashtbl.replace by_hash e.hash e)
                  entries;
                let cascaded =
                  List.filter_map
                    (fun (e : Day11_lib.Dag_marshal.entry) ->
                ...
                let cascade =
                  if cascaded = [] then []
                    ...
                    let row ((e : Day11_lib.Dag_marshal.entry), src) =
                      ...
                      tr
                        [
                          td [ pkg_link (OpamPackage.to_string e.pkg) ];
                          td [ txt (kind_label e.kind) ];
                    ...
                    List.iter
                      (fun (e, src) ->
                        let prev =
                          try Hashtbl.find by_root src with Not_found -> []
                        in
                        Hashtbl.replace by_root src ((e, src) :: prev))
                      cascaded;
                    let groups =
                      Hashtbl.fold
                        (fun root rs acc -> (root, rs) :: acc)
                        by_root []
                    in
                    let groups =
                      List.sort
                      ...
                      p
                        [
                          em
                            [
                              txt
                                "Nodes that didn't run because an upstream \
                                 node failed.";
                            ];
                        ];
                      table
        ...
        timing "pkg_table" (fun () ->
            ...
            | Some (entries, cascade_table) ->
             ...
             behind a successful build. *)
                ...
                in
                let kind_bucket :
                    Day11_lib.Dag_marshal.kind -> [ `Build | `Doc | `Skip ] =
                  function
                  | Build | Tool -> `Build
               ...
               with no doc kinds), shown as "—". *)
                  if sts = [] then None
                    ...
                    List.exists (fun s -> s = Day11_lib.Cascade.Failed) sts
                  then Some "failed"
                  else if
                    List.exists
                      (function
                        | Day11_lib.Cascade.Cascade _ -> true | _ -> false)
                      sts
                  then Some "cascade"
                  else if
                    List.exists (fun s -> s = Day11_lib.Cascade.Pending) sts
                  then Some "pending"
                  else Some "ok"
                in
                let cell sts =
                  match aggregate sts with
            ...
            | None -> (
                let pkg_row p =
                  let status_cell =
                    match latest_pkg_status snapshot_dir p with
                  ...
                  | _ ->
                      table
                        ...
                        ~thead:
                          (thead
                             [
                               tr [ th [ txt "Package" ]; th [ txt "Status" ] ];
                             ])
      ...
      let cascade_fold =
        match cascade_section with
        | [] -> []
        | content -> [ details (summary [ txt "Cascaded nodes" ]) content ]
      in
                ...
                | _ ->
                    let rows =
                      List.map
                        (fun pkgver ->
                          let cell =
                            match String.index_opt pkgver '.' with
                            | Some i ->
                                let n = String.sub pkgver 0 i in
                                let v =
                                  String.sub pkgver (i + 1)
                                    (String.length pkgver - i - 1)
                                in
                                a
                                  ~a:
                                    [
                                      a_href
                                        (Printf.sprintf "/profiles/%s/p/%s/%s"
                                           name n v);
                                    ]
                                  [ txt pkgver ]
                            | None -> txt pkgver
                    ...
                    [
                      details
                        (summary
                           [
                             txt
                               (Printf.sprintf "Solve failures (%d)"
                                  (List.length pkgs));
                           ])
                        [
                          p
                            [
                              em
                                [
                                  txt
                                    "Targets with no dependency solution — \
...
type pkg_change =
  | Added of string * string * string
      (** [(version, status, build_hash)] — fresh package in the new snapshot.
          [build_hash] is the dispatched build's hash so the status cell can
          deep-link to logs / docs; empty string when unknown. *)
  | Removed of string  (** [version] — package present in old, gone in new. *)
  | Status_changed of string * string * string * string
      (** [(version, old_status, new_status, new_build_hash)] — same (name,
          version), different status. *)
  | Version_changed of string * string * string * string
      (** [(old_version, new_version, new_status, new_build_hash)] — single old
          version replaced by a single new version (collapsed Removed+Added). *)
...
let make_load_diff_pkgs_memo () =
  let cache : (string, ((string * string) * (string * string)) list) Hashtbl.t =
    Hashtbl.create 32
  in
  fun snapshot_dir ->
...
let compute_diff_changes m_old m_new =
  ...
  let classify =
    List.filter_map
      ...
      keys
  in
  let counts : (string, int ref * int ref) Hashtbl.t = Hashtbl.create 16 in
  List.iter
    (fun (n, k) ->
      let r, a =
        try Hashtbl.find counts n
        with Not_found ->
  ...
  List.iter
    (fun (n, k) ->
      ...
      | Some (r, a) when !r = 1 && !a = 1 -> (
          ...
          | `Removed v_old, None -> Hashtbl.add paired n (v_old, "", "", "")
...
let render_change_row ~profile_name ~html_dir (name, change) =
  let pkg_link =
    a
      ~a:[ a_href (Printf.sprintf "/profiles/%s/p/%s" profile_name name) ]
      [ txt name ]
  in
  let ver_link v =
    a
      ~a:[ a_href (Printf.sprintf "/profiles/%s/p/%s/%s" profile_name name v) ]
      [ txt v ]
  in
    ...
    let cmd =
      Printf.sprintf
        "git -C %s log -n 500 --no-decorate \
         --pretty=format:%%h%%x1f%%ad%%x1f%%an%%x1f%%s --date=short %s \
         2>/dev/null"
        (Filename.quote repo)
...
let repo_changes_section ~dir_old ~dir_new =
  (* Read the log oldest -> newest regardless of which snapshot is the
     ...
     chronological). *)
  let dir_old, dir_new =
    if snapshot_created dir_old <= snapshot_created dir_new then
      (dir_old, dir_new)
    else (dir_new, dir_old)
  in
      ...
      | Some old_commit ->
          ...
          let header =
            h3
              [
                txt
                  (Printf.sprintf "%s  %s..%s" repo_name
                     (Templates.short_sha old_commit)
                     (Templates.short_sha new_commit));
              ]
          in
          let body =
            match rows with
                  ...
                  p
                    [
                      em
                        [
                          txt
                            "No commits in this range (the newer snapshot's \
            ...
            | _ ->
                let truncated =
                  if List.length rows >= 500 then
                    [ p [ em [ txt "(showing first 500 commits)" ] ] ]
                  else []
                in
                truncated
                @ [
...
let snapshot_diff ~ctx name key_old key_new =
  object
    ...
    method! private get web_ctx =
      ...
      let incomplete =
        List.filter
          (fun (_key, dir) -> not (snapshot_completed dir))
              ...
              (fun (key, _) ->
                p
                  ...
                  [
                    ...
                    Templates.sha_span key;
                    txt
                      " is incomplete — its run hasn't finished (or was \
...
let recent_changes ~ctx name =
  object
    ...
    method! private get web_ctx =
      ...
      let snaps =
        list_snapshots_newest_first ctx name |> List.filter snapshot_completed
      in
...
(** Percent-encode a string for use in a URL query (GitHub issue
    title/body/search). Conservative: only unreserved chars pass through,
    everything else (incl. newlines and spaces) is %XX. *)
let urlencode s =
...
(** Substring test without allocation (used to pre-filter big [build.jsonl]
    lines before JSON-parsing). *)
let str_contains s sub =
  ...
  if m = 0 then true
    ...
    in
    let rec loop i =
      if i + m > n then false else if at i 0 then true else loop (i + 1)
    in
    loop 0

(** Minimal HTML escape for text interpolated into raw-HTML [Unsafe.data]
    fragments (dep names, hashes). *)
let esc_html s =
...
let read_solver_failure snapshot_dir pkg_str =
  match
    Bos.OS.File.read Fpath.(snapshot_dir / "solutions" / (pkg_str ^ ".json"))
  with
  | Error _ -> None
  | Ok data -> (
      match try Some (Yojson.Safe.from_string data) with _ -> None with
...
let package_version ~ctx name pkg ver =
  object
    ...
    method! private get web_ctx =
        ...
        if entries <> [] then entries
          ...
          List.concat_map
            (fun snap ->
              ...
              | Ok data -> (
                  match
                    try Some (Yojson.Safe.from_string data) with _ -> None
                  with
                  | Some (`List nodes) ->
                      let open Yojson.Safe.Util in
                      let parsed =
                        List.filter_map
                          (fun j ->
                            ...
                            | Some hash ->
                                ...
                                in
                                Some (hash, kind, universe, blessed))
                          nodes
                      in
                   ...
                   not the (pending) docs. *)
                      let pkg_blessed =
                        List.exists (fun (_, _, _, b) -> b) parsed
                      in
                      List.filter_map
                        (fun (hash, kind, universe, blessed) ->
                          if Hashtbl.mem seen hash then None
                            ...
                            let status, category =
                              ...
                              | Some 0 ->
                                  ( "success",
                                    if is_doc then "doc_success" else "success"
                                  )
                              | Some _ ->
                                  ( "failure",
                                    if is_doc then "doc_failure"
                                    else "build_failure" )
                              | None ->
                                  ("pending", if is_doc then "doc" else "build")
                            in
                            let blessed =
                              if is_doc then blessed else pkg_blessed
                            in
                            Some
                              {
                                Day11_lib.History.ts = "";
                                run = "(plan)";
                                build_hash = hash;
                                status;
                                category;
      ...
      let job_ids =
        job_ids_for_hashes
          (List.map (fun (e : Day11_lib.History.entry) -> e.build_hash) entries)
      in
      let job_id_of bh =
        if String.length bh = 0 then None
        else
          Hashtbl.find_opt job_ids (String.sub bh 0 (min 12 (String.length bh)))
      in
      let crumbs =
        Templates.breadcrumbs
          [
            ...
            List.iter
              (fun (e : Day11_lib.History.entry) ->
                if
                  ...
                  && not (Hashtbl.mem tbl e.build_hash)
                then
                  let layer_dir =
                    Day11_layer.Layer.(dir (of_hash ~os_dir e.build_hash))
                  in
        ...
        List.map
          (fun (e : Day11_lib.History.entry) ->
           ...
           job_id (e.g. for entries pre-dating the SQLite cache). *)
            let hash_cell =
              let target =
                match job_id_of e.build_hash with
                | Some job_id -> "/job/" ^ job_id
                | None ->
                    Printf.sprintf "/profiles/%s/builds/%s/log" name
                      e.build_hash
            ...
            let deps_cell =
              if is_doc_entry then td [ em [ txt "—" ] ]
                ...
                | Some deps ->
                    let deps_li =
                      String.concat ""
                        (List.map (fun d -> "<li>" ^ esc_html d ^ "</li>") deps)
                    in
                    td
                      [
                        Unsafe.data
                          (Printf.sprintf
                             "<label title=\"select to compare\"><input \
                              type=\"checkbox\" class=\"bd-sel\" value=\"%s\"> \
                              </label><details><summary>%d \
                              deps</summary><ul>%s</ul></details>"
      ...
      let latest_where pred =
        ...
        |> List.sort (fun (a : Day11_lib.History.entry) b -> compare b.ts a.ts)
        |> function
        | [] -> None
        | x :: _ -> Some x
      in
      let blessed_build =
        latest_where (fun (e : Day11_lib.History.entry) ->
            (not (is_doc_cat e)) && e.blessed)
      in
      let blessed_doc =
        latest_where (fun (e : Day11_lib.History.entry) ->
            is_doc_cat e && e.blessed)
      in
      let job_link bh label =
        let target =
          match job_id_of bh with
          | Some job_id -> "/job/" ^ job_id
      ...
      let os_label =
        ...
        | Ok (p : Profile.t) ->
            Printf.sprintf "%s %s"
              (String.capitalize_ascii p.os_distribution)
              p.os_version
        | Error _ -> "this platform"
      in
      let report_affordance ~repo ~title ~body ~lead =
        let new_url =
          Printf.sprintf "https://github.com/%s/issues/new?title=%s&body=%s"
            repo (urlencode title) (urlencode body)
        in
        let search_url =
          Printf.sprintf "https://github.com/%s/issues?q=%s" repo
            (urlencode ("is:issue " ^ pkg))
        in
        p
          [
            txt lead;
      ...
      let page_url =
        ...
        | Some host ->
            let proto =
              match Cohttp.Header.get headers "x-forwarded-proto" with
              | Some p when p <> "" -> p
              | _ -> "https"
            in
                ...
                (try
                   while true do
                    ...
                    [failed_dep] is the package itself — skip those. *)
                     if
                       str_contains line pkg_needle
                       && str_contains line "\"status\":\"cascade\""
                       && str_contains line "\"kind\":\"build\""
                     then
                 ...
                 with End_of_file -> ());
                !found)
          (* last match in the file = most recent *)
        in
        let snap_dep snap =
      ...
      let cascade_blurb dep =
        let dep_link =
          ...
          | None ->
              a
                ~a:[ a_href (Printf.sprintf "/profiles/%s/p/%s" name dep) ]
                [ txt dep ]
          | Some i ->
        ...
        [
          div
            ...
            [
              p
                [
                  span ~a:[ a_class [ "cascade" ] ] [ txt "⚠ Not built" ];
                  txt
                    (Printf.sprintf
              ...
              p
                [
                  txt "This is usually not a problem with ";
                  txt pkg;
                  txt
                    " itself — it should build once the dependency does. \
      ...
      let status_block =
        if docs_present then
                ...
                a
                  ~a:
                    [
                      a_href
                        (Printf.sprintf
                           "/profiles/%s/docs/p/%s/%s/doc/index.html" name pkg
                           ver);
                    ]
              ...
              [
                div
                  ...
                  ([
                     p
                       [
                         span ~a:[ a_class [ "fail" ] ] [ txt "✗ Build failed" ];
                         txt
                           (Printf.sprintf " — %s."
                              (build_fail_phrase bb.category));
                       ];
                     p
                       [
                         txt "Blessed build job: ";
                  ...
                  @ [
                      report_affordance ~repo:"ocurrent/ocaml-docs-ci"
                        ...
                        ~lead:
                          (Printf.sprintf
                             "If you believe %s should compile correctly on \
                              %s, please comment on the ocurrent/ocaml-docs-ci \
                              issues:"
                             pkg_str os_label);
                    ]);
              ]
          | _, Some bd when bd.status = "failure" ->
              let univ =
                if bd.universe = "" then "(unknown)" else bd.universe
              in
              let body =
                Printf.sprintf
                  "Documentation generation failed for %s, although the \
                   package built successfully.\n\n\
                   Universe: %s\n\
                   Doc job hash: %s\n\
                   Profile: %s\n\
                   Page: %s\n"
                  pkg_str univ bd.build_hash name page_url
              in
              [
                div
                  ~a:[ a_class [ "warn" ] ]
                  [
                    p
                      [
                        span ~a:[ a_class [ "fail" ] ] [ txt "⚠ Docs failed" ];
                        txt
                          " — the package built, but odoc failed to generate \
                           documentation.";
                      ];
                    p
                      ([
                         txt "Blessed docs job: ";
                        ...
                        [
                          ...
                          a
                            ~a:
                              [
                                a_href
                                  (Printf.sprintf "/profiles/%s/u/%s" name
                                     bd.universe);
                              ]
                    ...
                    report_affordance ~repo:"ocaml/odoc"
                      ~title:
                        (Printf.sprintf "Doc generation failure: %s" pkg_str)
                      ~body
                      ~lead:
                        (Printf.sprintf
                           "If you believe %s's documentation should build, \
                            please comment on the ocaml/odoc issues:"
                           pkg_str);
                  ];
              ]
                    ...
                    div
                      ...
                      [
                        p
                          [
                            span
                              ...
                              [ txt "⚠ Docs failed" ];
                            txt
                              " — the package built, but the last completed \
                        ...
                        p
                          [
                            txt
                              "No doc job was logged for it, so the node \
                        ...
                        p
                          [
                            em
                              [
                                txt
                                  "Not documented in the last completed run: a \
                                   dependency failed, so this package's doc \
                                   node was skipped.";
                              ];
                          ];
                      ])
                    ...
                    p
                      [
                        em
                          [
                            txt
                              "No rendered docs found on disk, though the \
                               latest blessed build succeeded — the output may \
                               still be syncing.";
                          ];
                      ];
                  ])
                    ...
                    p
                      [
                        em
                          [
                            txt
                              "Not built in the latest run: a build dependency \
...
|script}
            ...
            Unsafe.data
              (Printf.sprintf
                 "<p class=\"crumbs\">Tick two or more build rows' checkboxes \
                  ...
                  differences.</p><div id=\"bd-cmp-result\"></div><script>var \
                  BD_DEPS=%s;\n\
                  %s</script>"
                 data_json js_logic);
          ]
      ...
      let solver_failure_block =
        match
          List.find_map (fun snap -> read_solver_failure snap pkg_str) snaps
        with
        | None -> []
        | Some err ->
            ...
            let err_for_body =
              if String.length err > 2000 then
                String.sub err 0 2000 ^ "\n…(truncated; see the page)"
              else err
            in
            let body =
              Printf.sprintf
                "Package %s could not be solved on %s — no dependency solution \
                 ...
                 Solver output:\n\
                 %s\n"
                pkg_str os_label name page_url err_for_body
            in
            [
              ...
              div
                ...
                [
                  p
                    [
                      span ~a:[ a_class [ "fail" ] ] [ txt "✗ Solve failed" ];
                      txt
                        " — no dependency solution was found for this package \
                         version on ";
                      txt os_label;
                      txt ", so it was never built. The solver's explanation:";
                    ];
                  pre [ txt err ];
                  report_affordance ~repo:"ocurrent/ocaml-docs-ci"
                    ...
                    ~lead:
                      (Printf.sprintf
                         "If you believe %s should be installable on %s, \
      ...
      in
      Context.respond_ok web_ctx
        ([ Templates.style_block; crumbs; h2 [ txt pkg_str ] ]
