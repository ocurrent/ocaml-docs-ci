...
module Stats = struct
  type status_count = { total : int; failed : int; pending : int; success : int }

  let to_string { total; failed; pending; success } =
    let pending = match pending with 0 -> "" | n -> Fmt.str "/ %d 🟠" pending in
    let failed = match failed with 0 -> "" | n -> Fmt.str "/ %d ❌" failed in
    Fmt.str "%d ↦ %d 📗 %s %s" total success pending failed

  type t = { opam : status_count; versions : status_count; universes : status_count }

  let empty =
    let empty_status () = { total = 0; failed = 0; pending = 0; success = 0 } in
    { opam = empty_status (); versions = empty_status (); universes = empty_status () }
  ...
  let update stats kind status =
    ...
    | Universe -> { stats with universes = update_status stats.universes status }
end
...
let get_cohttp (type a) (module Api : S with type t = a) ~api () =
  ...
  let* response, body = Cohttp_lwt_unix.Client.post ~headers ~body:(`String serialized_body) api in
  let* body = Cohttp_lwt.Body.to_string body in
  match Cohttp.Code.(code_of_status response.status |> is_success) with
  | false -> Lwt.fail (Failure ("Status: " ^ Cohttp.Code.string_of_status response.status))
  | true ->
...
let parse_status t =
  ...
  | `FutureAddedValue _ -> failwith "Unsupported value"
...
let parse data =
  ...
  let data =
    data.packages |> Array.to_seq |> Seq.flat_map (parse_package ~stats) |> OpamPackage.Map.of_seq
  in
  (data, !stats)
  ...
  let rec updater () =
    let* () =
      Lwt.catch
        ...
        (fun exn ->
          Dream.error (fun log ->
              log "Failed to contact the graphql api: %s" (Printexc.to_string exn));
          Lwt.return ())
    in
...
let extract_docs_body html =
  ...
  let preamble =
    soup |> select_one ".odoc-preamble" |> Option.map to_string |> Option.value ~default:""
  in
  let toc = soup |> select_one ".odoc-toc" |> Option.map to_string |> Option.value ~default:"" in
  let content =
    soup |> select_one ".odoc-content" |> Option.map to_string |> Option.value ~default:""
  in
...
let load t path =
  let res () =
    ...
    if Astring.String.is_suffix ~affix:".html" path then try_load ~t path
      ...
      | Error _ -> try_load ~t (Astring.String.trim ~drop:(( = ) '/') path ^ ".html")
  in
  res ()
