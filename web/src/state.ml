  ...
  let rec updater () =
    let* () =
      Lwt.catch
        ...
        (fun exn ->
          Dream.error (fun log ->
              log "Failed to update the opam package list: %s"
...
let v ~prefix ~api ~polling () =
  Dream.log "Initializing state";
  let docs = Documentation.make ~api ~polling () in
  let v = { packages = OpamPackage.Name.Map.empty; docs; prefix } in
  Lwt.async (fun () -> poll_for_opam_packages ~polling v);
  v
...
let get_package_gql t name =
  t.packages |> OpamPackage.Name.Map.find name
